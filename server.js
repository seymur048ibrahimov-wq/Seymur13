...
// Fallback (AUTO_TOP30 uğursuz olsa və ya SCAN_SYMBOLS yazılmasa istifadə olunan) — ancaq təsdiqlənmiş 30 coin,
// başqa heç bir simvol bu siyahıya daxil edilməməlidir.
const DEFAULT_SYMBOLS = [
  ...PRIORITY_SYMBOLS,
  'ADA-USDT-SWAP','SUI-USDT-SWAP','PEPE-USDT-SWAP','HYPE-USDT-SWAP','PUMP-USDT-SWAP',
  'LTC-USDT-SWAP','LINK-USDT-SWAP','TAO-USDT-SWAP','TRX-USDT-SWAP','ZEC-USDT-SWAP',
  'AAVE-USDT-SWAP','APT-USDT-SWAP','ARB-USDT-SWAP','DOT-USDT-SWAP','ENA-USDT-SWAP',
  'FIL-USDT-SWAP','HBAR-USDT-SWAP','ICP-USDT-SWAP','NEAR-USDT-SWAP','ONDO-USDT-SWAP',
  'SHIB-USDT-SWAP','TON-USDT-SWAP'
];
// İcazə verilən TAM universum — AUTO_TOP30 belə bu 30 coindən kənara çıxa bilməz.
...
let symbols = AUTO_TOP30
  ...
  : rawScanEnv.split(',').map(x=>x.trim()).filter(Boolean).filter(s=>{
      const ok = ALLOWED_SYMBOLS.has(s);
      if(!ok) console.warn(`SCAN_SYMBOLS: "${s}" icazə verilən 30 coin siyahısında deyil, ötürüldü.`);
      return ok;
    });
if(!symbols.length) symbols = DEFAULT_SYMBOLS.slice();
...
async function discoverTop30(){
  try{
    ...
    // Yalnız təsdiqlənmiş 30 coin universumundan (ALLOWED_SYMBOLS) seçilir — başqa heç bir simvol daxil edilmir
    const usdt = d.filter(t=>t.instId && ALLOWED_SYMBOLS.has(t.instId));
    usdt.sort((a,b)=>Number(b.volCcy24h||0)-Number(a.volCcy24h||0));
    const top = usdt.map(t=>t.instId);
